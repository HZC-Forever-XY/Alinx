    ...
    def __init__(self):
        ...
        while not (is_exit and rospy.is_shutdown()):
            try:
                ...
                i += 1
                # response = {'latitude':self.position.latitude, 'longitude':self.position.longitude, 'velocity':self.velocity, 'heading':self.heading.data,'wind_direction_apparent':self.wind_direction_apparent.data}
                response = {'latitude':self.lati.data, 'longitude':self.long.data, 'velocity':self.velocity.data, 'heading':self.heading.data,'wind_direction_apparent':self.wind_direction_apparent.data}
